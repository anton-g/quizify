...
.quiz-lobby
  ...
  a.button.is-light.is-outlined.is-fullwidth(@click="leaveQuiz") {{ $t('leave-button') }}
</template>
...
export default {
  ...
  created () {
    console.log('joined')
    if (!this.$store.state.player.user.id) {
  ...
  computed: {
    ...
    currentPlayerId () {
      return this.$store.state.player.user.id
    },
    quizStarted () {
...
<i18n>
{
  "en": {
    ...
    "leave-button": "Leave quiz"
  },
  "sv": {
    "heading": "Lobby",
    "waiting": "Väntar på att spelet ska börja..",
    "leave-button": "Lämna quiz"
  }
}
</i18n>
